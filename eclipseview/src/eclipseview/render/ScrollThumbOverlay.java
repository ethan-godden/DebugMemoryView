...
/**
 * Auto-hiding overlay scroll thumbs (macOS-editor style). The panes' stock
 * scrollbars are ScrollPane.NEVER; while an axis is actively scrolling this
 * draws a slim rounded semi-transparent thumb on a dedicated mouse-transparent
 * layer along the pane's right edge (bottom edge for a horizontal axis), then
 * fades it out after a short idle. Thumbs are display-only — wheel and
 * click-to-reveal remain the scrolling affordances. Geometry comes from the
 * axis RangeModel and the pane's absolute client area at show-time, so a live
 * scroll re-positions the thumb on every value change.
 */
final class ScrollThumbOverlay {
    ...
    void track(ScrollPane pane, boolean vertical) {
        Thumb thumb = new Thumb(pane, vertical);
        thumbs.add(thumb);
        layer.add(thumb.figure);
        model(pane, vertical).addPropertyChangeListener(event -> {
    ...
    void show(ScrollPane pane, boolean vertical) {
        for (Thumb thumb : thumbs) {
            if (thumb.pane == pane && thumb.vertical == vertical) {
                thumb.show();
            }
        }
    }
    ...
    private static RangeModel model(ScrollPane pane, boolean vertical) {
        return vertical ? pane.getViewport().getVerticalRangeModel()
                : pane.getViewport().getHorizontalRangeModel();
    }
    ...
    private final class Thumb {
        ...
        Thumb(ScrollPane pane, boolean vertical) {
            this.pane = pane;
        ...
        private Rectangle thumbBounds() {
            RangeModel model = model(pane, vertical);
            int range = model.getMaximum() - model.getMinimum();
            int extent = model.getExtent();
            if (extent <= 0 || range <= extent) {
                return null;
            }
            Rectangle visible = RevealUtil.absoluteClientArea(pane.getViewport());
